...
local LanguageOptions = Class(Screen, function(self)
	...
	self.backbutton:SetText(STRINGS.UI.CONTROLSSCREEN.CLOSE)
	...
	self.URLButton:SetOnClick( function() VisitURL(RussificationURL) end )
    ...
    self.version:SetString("Версия: "..RussificationVersion)

    self.updateoptionstext = self.updateoptionspanel:AddChild(Text(BUTTONFONT, 35))
	...
	for i,v in ipairs(UpdatePeriod) do
		if v==period then
	...
	self.updateoptionsspinner.OnChanged =
		function( _, data )
			...
			Profile:SetLocalizaitonValue("update_frequency",UpdatePeriod[self.updateoptionsspinner:GetSelectedIndex()])
		end
	...
	if not kleifileexists(russianmodpath.."history.txt") then
	...
	self.UpdateOptionsDescriptionText:SetColour(.9,.9,.9,1)
	...
	local function DoDownload(...)
		...
		if ... then --пустые они будут если сервер не вернёт ответа или в самый первый раз
			...
			if tbl and #tbl==3 and tbl[3]==200 then --получили ответ от сервера и он положителен
				...
				if id and head then
					VisitURL(RussificationURL..head.."/"..id)
				end
			end
		end
	end
				...
				self.logtextbg[i].OnGainFocus=function(self2)
					...
					if self.urlbutton then
						...
						if not self.ControlButtonPressed then
							self.urlbutton:SetOnClick( function() VisitURL(RussificationURL..chapterslist[self2.chapter].id.."?show=5&to="..self2.tag) end )
						else
							self.urlbutton:SetOnClick( function()
								TheSim:QueryServer( RussificationURL..chapterslist[self2.chapter].id.."?show=5&to="..self2.tag, DoDownload, "GET" )
...
function LanguageOptions:LoadHistory()
	...
	self.logtextlines=0
	local lines=0
	local f=io.open(russianmodpath.."history.txt","r")
...
function LanguageOptions:LoadUpdateLog()
	...
	self.UpdateLog={}
	if not kleifileexists(RussificationStorePath..RussianUpdateLogFileName) then
		return nil
	end
	...
	local f=io.open(RussificationStorePath..RussianUpdateLogFileName,"r")
	if f then for line in f:lines() do 
...
function LanguageOptions:OnControl(control, down)
	...
	elseif control == CONTROL_ACTION and self.updateoptionsspinner.focus then
		TheFrontEnd:GetSound():PlaySound("dontstarve/HUD/click_move")
		if self.updateoptionsspinner:GetSelectedIndex()<#self.updateoptions then
			self.updateoptionsspinner:Next(true)
		else
			self.updateoptionsspinner:SetSelectedIndex(1)
		end	CONTROL_FOCUS_UP = 25
